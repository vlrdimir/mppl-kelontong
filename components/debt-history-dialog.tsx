...
export function DebtHistoryDialog({ debt }: DebtHistoryDialogProps) {
  ...
  const payments = debt.debtPayments || []
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Riwayat pembayaran piutang {debt.customer?.name}</DialogDescription>
        </DialogHeader>

        <div className="space-y-2 py-4 border-y">
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Total Hutang:</span>
            <span className="font-semibold">{formatCurrency(debt.totalDebt)}</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-red-600">{formatCurrency(debt.remainingDebt)}</span>
          </div>
        </div>
