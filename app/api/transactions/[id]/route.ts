...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { paymentStatus, paidAmount, notes } = body as Partial<{ paymentStatus: string; paidAmount: string; notes: string }>

    // Ambil transaksi saat ini untuk validasi
    const current = await db.query.transactions.findFirst({ where: eq(transactions.id, id) })
    if (!current) {
      return NextResponse.json({ error: "Transaction not found" }, { status: 404 })
    }

    // Calon nilai baru
    const newStatus = typeof paymentStatus !== "undefined" ? paymentStatus : (current.paymentStatus as string)
    const newPaid = typeof paidAmount !== "undefined" ? Number(paidAmount) : Number(current.paidAmount ?? 0)
    ...
    if (trx) {
      ...
      const existingDebt = await db.query.debts.findFirst({ where: eq(debts.transactionId, id) })
      ...
      if (remaining > 0) {
        ...
        // Tandai piutang lunas jika tidak ada sisa
        await db
          .update(debts)
          .set({ paidAmount: String(paid), remainingDebt: "0", status: "paid", updatedAt: new Date() })
          .where(eq(debts.id, existingDebt.id))
      }
    }
